...
static int ct_panel_prepare(struct drm_panel *panel)
{
	...
	if (ct->reset != NULL)
	gpiod_set_value(ct->reset, 1); /*--reset */
	udelay(5);	
...
extern void mipi_dsi_read_reg(struct mipi_dsi_device *dsi,u8 addr);
...
static int ct_panel_enable(struct drm_panel *panel)
{
	...
	mipi_dsi_read_reg(dsi,0x00);
	mipi_dsi_read_reg(dsi,0x01);
	mipi_dsi_read_reg(dsi,0x02);
	/* Software reset */
