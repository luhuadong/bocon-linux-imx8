...
/*读寄存器*/
int mipi_dsi_read_reg(struct mipi_dsi_device *dsi,u8 addr,uint8_t *val)
{
	u8 date;
	ssize_t ret;
...
写8位的寄存器
*/
int mipi_dsi_write_reg(struct mipi_dsi_device *dsi,uint8_t addr, uint8_t val)
{
	int ret; 
...
写16位到连续地址的寄存器
*/
int mipi_dsi_write_u16(struct mipi_dsi_device *dsi,uint8_t reg,uint16_t val)
{
	int rl,rh;
...
/*更新寄存器的某些位*/
int mipi_dsi_reg_up_bit(struct mipi_dsi_device *dsi,uint8_t addr,uint8_t mask,
uint8_t val)
{
	int ret;
...
static int ct_panel_push_cmd_list(struct mipi_dsi_device *dsi)
{
	...
	/* 设置DSI时钟范围 DP 时钟范围 */
	mipi_dsi_write_reg(dsi, SN_DSIA_CLK_FREQ_REG, 0x53); /*[REG:0x12]*/				
	...
	//mipi_dsi_reg_up_bit(dsi,SN_HPD_DISABLE_REG,HPD_DISABLE,0);
